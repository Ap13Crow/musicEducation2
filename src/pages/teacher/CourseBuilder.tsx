...
export default function CourseBuilder() {
  ...
  const selected = courses.find(c => c.title === selectedCourse);

  const courseModules = modules
    .filter(m => selectedCourse && getFieldValue(m.node, 'Course') === selectedCourse)
    .sort((a, b) => a.order - b.order);

  const courseLessons = lessons
    .filter(l => courseModules.some(m => m.title === l.moduleRef))
    .sort((a, b) => a.order - b.order);

  const courseMedia = media.filter(m => m.course === (selected?.title ?? ''));
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setMedia(allMedia);
      if (teacherCourses.length > 0 && !selectedCourse) setSelectedCourse(teacherCourses[0].title);
    } catch (err) { console.error('[CourseBuilder]', err); }
    finally { setLoading(false); }
  }, [teacherName, selectedCourse]);
  ...
  async function handleAddLesson(moduleTitle: string) {
    if (!newLessonTitle.trim()) return;
    setSaving(true);
    try {
      const modLessons = courseLessons.filter(l => l.moduleRef === moduleTitle);
    ...
    <div className="p-6 lg:p-8 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-3">
          <select
            ...
            className="text-sm border border-border bg-background rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary/20"
          >
            {courses.map(c => <option key={c.node.id} value={c.title}>{c.title}</option>)}
          </select>
          <button onClick={() => selected && navigate(`/my-courses/${selected.node.id}`)} className="flex items-center gap-2 px-4 py-2 rounded-lg border border-border text-sm font-medium hover:bg-muted/50 transition-colors">
      ...
      {courses.length === 0 ? (
        <div className="text-center py-16 rounded-xl border border-border bg-card">
          ...
          <p className="text-muted-foreground mt-3">No courses found. Create a course first, or ask the Course Architect agent to help you build one.</p>
        </div>
      ) : (
        <>
          {selected && (
            ...
            <div className="space-y-3">
              ...
              {courseModules.length === 0 && !showAddModule ? (
                ...
                courseModules.map(mod => {
                  const modLessons = courseLessons.filter(l => l.moduleRef === mod.title).sort((a, b) => a.order - b.order);
                          ...
                          <div className="divide-y divide-border">
                            ...
                            {showAddLesson === mod.title ? (
                              <div className="p-4 space-y-3 bg-muted/10">
                                ...
                                <button onClick={() => handleAddLesson(mod.title)} disabled={saving || !newLessonTitle.trim()} className="px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 disabled:opacity-50 transition-colors">{saving ? 'Saving...' : 'Add Lesson'}</button>
                              </div>
                            ) : (
                              <button onClick={() => { setShowAddLesson(mod.title); setNewLessonType('Reading'); setNewLessonDuration(''); setNewLessonMedia(''); setNewLessonMxml(''); }} className="w-full flex items-center gap-2 px-4 py-2.5 text-xs font-medium text-muted-foreground hover:text-foreground hover:bg-muted/30 transition-colors">
