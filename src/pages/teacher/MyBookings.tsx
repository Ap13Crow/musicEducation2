...
import { CalendarDays, Clock, Music, CheckCircle2, Circle, XCircle, AlertTriangle, MapPin, ExternalLink, Video, Trash2, Check } from 'lucide-react';
...
export default function TeacherMyBookings() {
  ...
  const loadBookings = useCallback(() => {
    getNodes(PROJECTS.bookings).then((nodes) => {
      setBookings(nodes.filter((b) => getFieldValue(b, 'Teacher') === teacherName));
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl">
      <header className="mb-8">
        ...
        <p className="text-muted-foreground mt-1">{bookings.length} lesson{bookings.length !== 1 ? 's' : ''} scheduled</p>
      </header>
              ...
              <div key={bk.id} className="rounded-xl border border-border bg-card p-4 hover:border-primary/10 transition-colors">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-sm truncate">{getFieldValue(bk, 'Student')}</h3>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2.5">
                      ...
                      {teacherEventId && <span className="inline-flex items-center gap-1 text-xs text-muted-foreground"><ExternalLink className="w-3 h-3" />In Calendar</span>}
                    </div>
                  </div>
                  <div className="flex items-start gap-2 shrink-0">
                    ...
                    {status === 'Scheduled' && (
                      <button onClick={() => handleStatusChange(bk.id, 'Confirmed')} disabled={actionId === bk.id} className="p-1.5 rounded-lg text-muted-foreground hover:text-emerald-400 hover:bg-emerald-500/10 transition-colors" title="Confirm booking">
                        <Check className="w-3.5 h-3.5" />
                      </button>
                    )}
                    {(status === 'Scheduled' || status === 'Confirmed') && (
                      <button onClick={() => handleStatusChange(bk.id, 'Cancelled')} disabled={actionId === bk.id} className="p-1.5 rounded-lg text-muted-foreground hover:text-rose-400 hover:bg-rose-500/10 transition-colors" title="Cancel booking">
